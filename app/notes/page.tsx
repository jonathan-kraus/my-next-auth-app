    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      {dueNotes > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-red-900 text-lg">📅 {dueNotes} Follow-up(s) Due Today</h3>
              <p className="text-red-700 text-sm">Check notes needing attention</p>
            </div>
          </div>
          <Link 
      ...
      {notes.length === 0 ? (
        ...
        <div className="grid gap-6">
          {notes.map(note => {
            const isOverdue = note.needsFollowUp && note.followUpDate && 
              new Date(note.followUpDate) <= new Date()
            
            return (
              <div key={note.id} className="border rounded-lg p-6 hover:shadow-lg transition-all">
